...
def get_goods_list():
    ...
    filter_condition.add(Good.title.like('%' + request.args.get('title') + '%'))
    filter_condition.add(Good.detail.like('%' + request.args.get('detail') + '%'))
...
def censor_good(good_id):
    ...
    Good.query.filter_by(good_id=good_id).update({'state': data.get('op')})
